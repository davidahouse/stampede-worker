...
async function prepareWorkingDirectory(taskExecutionConfig, conf) {
  ...
  const todayFolder = today.getMonth().toString() + '-' + today.getDay().toString() +
        '-' + today.getFullYear().toString()
  const dir = conf.workspaceRoot + '/' + todayFolder + '/' +
        taskExecutionConfig.task.owner + '-' + taskExecutionConfig.task.repository + '/' +
        taskExecutionConfig.task.buildKey + '/' +
        taskExecutionConfig.task.buildNumber.toString() + '/' +
        taskExecutionConfig.task.task.id + '-' + taskExecutionConfig.task.task.number
  ...
  if (taskExecutionConfig.gitClone === 'ssh' || taskExecutionConfig.gitClone === 'https') {
    ...
    if (taskExecutionConfig.gitClone === 'ssh') {
      ...
      await cloneRepo(taskExecutionConfig.task.scm.cloneURL, dir,

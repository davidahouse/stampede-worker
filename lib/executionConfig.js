...
async function prepareExecutionConfig(task, conf) {
    ...
    return {
        ...
        taskArguments: workerConfig.taskArguments != null ?
            workerConfig.taskArguments.split(' ') :
            [],
        gitClone: workerConfig.gitClone != null ?
        workerConfig.gitClone :
            conf.gitClone,
        gitCloneOptions: workerConfig.gitCloneOptions != null ?
            workerConfig.gitCloneOptions :
            conf.gitCloneOptions,
        environmentVariablePrefix: workerConfig.environmentVariablePrefix != null ?
            workerConfig.environmentVariablePrefix :
            conf.environmentVariablePrefix,
        shell: workerConfig.shell != null ?
            workerConfig.shell :
            conf.shell,
        stdoutLogFile: workerConfig.stdoutLogFile != null ?
            workerConfig.stdoutLogFile :
            conf.stdoutLogFile,
        stderrLogFile: workerConfig.stderrLogFile != null ?
...
async function localTaskConfig(task, conf) {
    try {
        const taskFileContents = fs.readFileSync(conf.stampedeConfigPath + '/tasks/' + task.task.id + '.yaml')
